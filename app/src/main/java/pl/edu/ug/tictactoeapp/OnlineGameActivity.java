...
public class OnlineGameActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        actualWins1 = myScore.getInt("actualWins1", 0);
    ...
    void CheckWinner() {
        ...
        if(winner != 0 && gameState == 1){
            if(winner == 1){
                ...
                actualWins1 += 1;
                SharedPreferences myScore1 = getSharedPreferences("MyAwesomeScore", Context.MODE_PRIVATE);
                SharedPreferences.Editor editor = myScore1.edit();
                editor.putInt("actualWins1", actualWins1);
                editor.commit();
                myRef.child("users").child(otherPlayer).child("points").setValue(myScore1);

            }else if(winner == 2){
                ShowAlert("You won the game");
                actualWins2 += 1;
                myRef.child("users").child(userName).child("points").setValue(actualWins2);
            }
            gameState = 2;
        }
    ...
    void ShowAlert(String Title){
        AlertDialog.Builder b = new AlertDialog.Builder(this, R.style.Theme_AppCompat_Dialog_Alert);
        b.setTitle(Title)
